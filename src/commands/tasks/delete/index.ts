...
const executeCommand: CommandExecutor = async () => {
  ...
  if (!listIndicator) {
    console.log(
      chalk.red(
        "Must specify which list the specified task is in. See `tasks delete --help`."
      )
    );
    return;
  }
  if (!id) {
    console.log(
      chalk.red("Must specify a id for the task. See `tasks delete --help`.")
    );
    return;
  }

  const { list, tasks } = await deleteTask(listIndicator, id);

  printTaskListItems(list, tasks);
  console.log();
};

const deleteTask = async (listIndicator: string, id: string) => {
