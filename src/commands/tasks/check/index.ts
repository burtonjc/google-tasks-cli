...
const executeCommand: CommandExecutor = async () => {
  ...
  if (!listIndicator) {
    console.log(
      chalk.red(
        "Must specify which list the specified task is in. See `tasks check --help`."
      )
    );
    return;
  }
  if (!id) {
    console.log(
      chalk.red("Must specify a id for the task. See `tasks check --help`.")
    );
    return;
  }

  const { list, tasks } = await checkTask(listIndicator, id);

  printTaskListItems(list, tasks);
  console.log();
};

const checkTask = async (listIndicator: string, id: string) => {
